...
完成基础闭环后，不必把知识库中的每条分类都当成并列的主攻方向。对于人形机器人运动控制算法开发，核心研究对象是同一个：在真机可获得的观测和可执行的控制接口下，形成能够保持平衡、产生运动、跟踪目标、应对扰动并从偏差中恢复的通用全身运动控制基座。

这个基座可以按策略主要接收的控制条件，分成两条控制主线。支持多种提示接口的跨界工作应按主要贡献理解，不做机械二分。

#### 自主运动、地形适应与行为先验

输入以速度、方向、目标位置或行为提示为主，策略不以追踪一条完整参考轨迹作为唯一目标，而是根据任务条件和身体状态自主产生运动。基础层解决站立、行走、跑跳、速度跟踪与抗扰；感知层加入深度图、高度图或地形编码，使身体根据台阶、斜坡和沟壑提前调整落脚与全身姿态；运动先验层再用动作数据、判别器、无监督技能表示或行为潜变量约束动作分布。视觉感知、复杂地形和AMP因此不是三个并列方向，而是同一自主运动主线上的不同问题。可依次阅读[DreamWaQ](../论文与项目/论文逐篇解读/P125.md)、[AMP](../论文与项目/论文逐篇解读/P022.md)和[BFM-Zero](../论文与项目/论文逐篇解读/P080.md)，分别理解状态估计、对抗运动先验和可提示行为基座在这条链路中的作用。

#### 参考驱动的全身跟踪与人体控制

输入是参考动作、关键点、目标姿态或实时人体信号，策略负责把参考转换成真机可执行的全身关节目标。研究重点是大规模动作跟踪、未见动作泛化、参考修正、动力学失配、碰撞扰动、延迟和偏离参考后的恢复。[SONIC](../论文与项目/论文逐篇解读/P035.md)代表大规模通用Tracker，[TWIST](../论文与项目/论文逐篇解读/P088.md)把当前人体信号、因果跟踪策略和真机遥操作连成闭环，[BeyondMimic](../论文与项目/论文逐篇解读/P034.md)则处理参考修正与生成控制。

#### 上下游能力怎样处理

- **动作恢复、重定向和数据处理**是运动基座的上游基础，决定参考动作是否可达、接触是否可信、训练数据是否可用，本身不必与控制策略并列为主攻方向。
- **LocoManip**是运动基座向物理任务的系统集成，需要再接入物体状态、视觉或力触觉、接触处理和任务成功判断。它检验运动基座能否服务真实操作，不是另一套脱离基座的低层控制理论。
- **世界模型、VLA与Agent**是按需接入的上层生成、预测与调度模块。必须明确它们输出的是动作块、轨迹、末端目标还是技能调用，以及运动基座怎样执行并返回反馈。
- **Sim2Real、安全与测试**是所有方向共同经过的工程闭环，不是完成算法研究之后才附加的一条路线。
